...
*/
var GnoWallet = class extends _gnolang_tm2_js_client.Wallet {
	constructor() {
		super();
		...
		require_defineProperty._defineProperty(
			...
			*/
			...
			async (to, funds, endpoint, fee) => {
				const amount = require_utility.fundsToCoins(funds);
				const sender = await this.getAddress();
				const txFee = fee ? fee : {
					gas_wanted: 60000n,
					gas_fee: require_utility.defaultTxFee
				};
				const sendMsg = {
					from_address: sender,
					to_address: to,
				...
				const tx = {
					messages: [{
						type_url: require_endpoints.MsgEndpoint.MSG_SEND,
						value: require_bank.MsgSend.encode(sendMsg).finish()
					}],
					fee: txFee,
					memo: "",
					signatures: []
				};
				const signedTx = await this.signTransaction(tx, require_utility.decodeTxMessages);
		...
		require_defineProperty._defineProperty(
			...
			*/
			...
			async (sessionKey, expiresAt, spendLimit, allowPaths, endpoint, spendPeriod = 0n, fee) => {
				...
				const tx = {
					messages: [{
						type_url: require_endpoints.MsgEndpoint.MSG_CREATE_SESSION,
						value: require_auth.MsgCreateSession.encode(createSessionMsg).finish()
					}],
					fee: txFee,
					memo: "",
					signatures: []
		...
		require_defineProperty._defineProperty(
			...
			*/
			...
			async (sessionKey, endpoint, fee) => {
				...
				const tx = {
					messages: [{
						type_url: require_endpoints.MsgEndpoint.MSG_REVOKE_SESSION,
						value: require_auth.MsgRevokeSession.encode(revokeSessionMsg).finish()
					}],
					fee: txFee,
					memo: "",
					signatures: []
		...
		require_defineProperty._defineProperty(
			...
			*/
			...
			async (endpoint, fee) => {
				...
				const tx = {
					messages: [{
						type_url: require_endpoints.MsgEndpoint.MSG_REVOKE_ALL_SESSIONS,
						value: require_auth.MsgRevokeAllSessions.encode(revokeAllSessionsMsg).finish()
					}],
					fee: txFee,
					memo: "",
					signatures: []
		...
		require_defineProperty._defineProperty(
			...
			*/
			...
			async (path, method, args, endpoint, funds, maxDeposit, fee) => {
				const amount = require_utility.fundsToCoins(funds);
				const maxDepositAmount = require_utility.fundsToCoins(maxDeposit);
				...
				const tx = {
					messages: [{
						type_url: require_endpoints.MsgEndpoint.MSG_CALL,
						value: require_vm.MsgCall.encode(callMsg).finish()
					}],
					fee: txFee,
					memo: "",
					signatures: []
		...
		require_defineProperty._defineProperty(
			...
			*/
			...
			async (gnoPackage, endpoint, funds, maxDeposit, fee) => {
				...
				const tx = {
					messages: [{
						type_url: require_endpoints.MsgEndpoint.MSG_ADD_PKG,
						value: require_vm.MsgAddPackage.encode(addPkgMsg).finish()
					}],
					fee: txFee,
					memo: "",
					signatures: []
		...
		require_defineProperty._defineProperty(
			...
			*/
			...
			async (gnoPackage, endpoint, funds, maxDeposit, fee) => {
				...
				const tx = {
					messages: [{
						type_url: require_endpoints.MsgEndpoint.MSG_RUN,
						value: require_vm.MsgRun.encode(runMsg).finish()
					}],
					fee: txFee,
					memo: "",
					signatures: []
	...
	static addRealm(realms) {
		...
		require_defineProperty._defineProperty(AugmentedWallet, "realms", currentRealms.concat(realms));
	...
	*/
	static async createRandom(options) {
		const wallet = await _gnolang_tm2_js_client.Wallet.createRandom(options);
	...
	*/
	static async fromSigner(signer) {
		const wallet = await _gnolang_tm2_js_client.Wallet.fromSigner(signer);
		const gnoWallet = new this();
		gnoWallet.signer = wallet.getSigner();
		return gnoWallet;
	}
	...
	*/
	static async fromMnemonic(mnemonic, options) {
		const wallet = await _gnolang_tm2_js_client.Wallet.fromMnemonic(mnemonic, options);
		const gnoWallet = new this();
		gnoWallet.signer = wallet.getSigner();
		return gnoWallet;
	}
	...
	*/
	static async fromPrivateKey(privateKey, options) {
		const wallet = await _gnolang_tm2_js_client.Wallet.fromPrivateKey(privateKey, options);
		const gnoWallet = new this();
		gnoWallet.signer = wallet.getSigner();
		return gnoWallet;
	}
	...
	*/
	static fromLedger(connector, options) {
		const wallet = _gnolang_tm2_js_client.Wallet.fromLedger(connector, options);
		const gnoWallet = new this();
		gnoWallet.signer = wallet.getSigner();
		return gnoWallet;
	}
};
require_defineProperty._defineProperty(GnoWallet, "realms", []);
//#endregion
exports.GnoWallet = GnoWallet;
